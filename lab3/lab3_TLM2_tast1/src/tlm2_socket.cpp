...
struct Initiator: sc_module
{
  ...
  SC_CTOR(Initiator)
  : socket("socket"),data(0)  // Construct and name socket
  {
  ...
  void thread_process()
  {
   ...
   //   if (cmd == tlm::TLM_WRITE_COMMAND) data = 0xFF000000 | i;
    	...
    	data = r;
       	...
       	if ( addr > 256 ) {
            SC_REPORT_ERROR("TLM-2", "Address exceed the MEMORY SIZE");
       	};
    ...
    sc_time delay5 = sc_time(5, SC_NS);
    ...
    for (int i = 0; i < 30; i ++)
      ...
      //   if (cmd == tlm::TLM_WRITE_COMMAND) data = 0xFF000000 | i;
         ...
         socket->b_transport( *trans, delay5 );  // Blocking transport call
...
struct Memory: sc_module
{
 ...
 /* SC_CTOR(Memory)
  ...
  Memory(sc_module_name _name, int size)
  {
